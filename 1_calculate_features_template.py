...
def calculate_features(problem_ids):
    ...
    for f in suite:
        ...
        for rep in range(REPITIONS):
            ...
            hc = calculate_hill_climbing_features(f, dim, L_BOUND, U_BOUND)
            grad = calculate_gradient_features(f, dim, L_BOUND, U_BOUND)
            fdc = calculate_fitness_distance_correlation(X, y)
            #si = calculate_sobol_indices_features(f, dim, L_BOUND, U_BOUND)
            #ls = calculate_length_scales_features(f, dim, L_BOUND, U_BOUND)
            ...
            #nodes, edges = compute_local_optima_network(f, dim, L_BOUND, U_BOUND)
            #lon = compute_lon_features(nodes, edges)
            res = {**hc, **grad, **fdc, **meta, **pca, **nbc, **disp, **ic, **distr, **limo, **cm_angle, **cm_conv, **cm_grad, **ela_conv, **ela_level, **ela_curvate, **ela_local}
            res['fid'] = fid
            res['dim'] = dim
            res['iid'] = iid
            res['rep'] = rep
